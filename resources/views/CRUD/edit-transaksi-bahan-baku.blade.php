...
@section('namaHalaman', 'Edit Transaksi Bahan Baku')
@section('namaOperator', $user->id_hak === 1 ? 'Owner' : 'Admin Produksi')
@section('judul', 'Edit Transaksi Bahan Baku')
...
@section('content')
    ...
    <section class="-mt-14 bg-white dark:bg-gray-900">
        <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
            ...
            <form action="{{ route('transaksi-bahan-baku.update', $transaksiBahanBaku->id) }}" method="POST">
                @csrf
                @method('PUT')
                {{-- date picker --}}
                ...
                @foreach ($detailTransaksiBahanBaku as $currentDetail)
                    <div class="grid gap-4 sm:grid-cols-2 sm:gap-6 mt-6 mb-6">
                        ...
                        <div>
                            ...
                            <input type="number" name="jumlah[{{ $currentDetail->id }}]]"
                                value="{{ $currentDetail->jumlah }}"
                        ...
                        <div>
                            ...
                            <input type="number" name="harga[{{ $currentDetail->id }}]]" id="harga"
                                value="{{ $currentDetail->harga }}"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"

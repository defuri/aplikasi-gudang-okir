        ...
        <div
                        ...
                        @foreach ($DetailPesanan as $data)
                                    ...
                                    <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                                        ...
                                        <form action="{{ route('pesanan.update', $data->id) }}" method="POST">
                                            ...
                                            <div class="grid gap-4 mb-4 sm:grid-cols-2">
                                                    ...
                                                    <select id="pelanggan_id" name="pelanggan_id" required
                                                        ...
                                                        @foreach ($pelanggan as $PelangganItem)
                                                            <option value="{{ $PelangganItem->id }}"
                                                                {{ $data->pesanan->pelanggan_id == $PelangganItem->id ? 'selected' : '' }}>
                                                                {{ $PelangganItem->nama }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                ...
                                                <div>
                                                    <label for="TanggalUpdate{{ $loop->iteration }}"
                                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tanggal</label>
                                                    <div class="relative max-w-sm">
                                                        <div
                                                            class="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
                                                        ...
                                                        @foreach ($pesanan as $PesananItem)
                                                            @if ($PesananItem->id == $data->pesanan_id)
                                                                <input datepicker datepicker-autohide type="text"
                                                                    required name="tanggal"
                                                                    id="TanggalUpdate{{ $loop->iteration }}"
                                                                    value="{{ date('d-m-Y', strtotime($PesananItem->tanggal)) }}"
                                                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                                                    placeholder="Pilih tanggal">
                                                            @endif
                                                        @endforeach
                                                    ...
                                                    <select id="produk_id" name="produk_id" required
                                                        ...
                                                        @foreach ($produk as $DataProduk)
                                                            <option value="{{ $DataProduk->id }}"
                                                                @if ($DataProduk->id == $data->produk_id) selected @endif>
                                                                {{ $DataProduk->nama }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                ...
                                <div class="relative p-4 w-full max-w-md max-h-full">
                                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                        ...
                                        <div class="p-4 md:p-5 text-center">
                                            ...
                                            <form action="{{ route('pesanan.destroy', $data->pesanan_id) }}"
                                                method="post"> {{-- * untuk hapus --}}
                                                @csrf
                                                @method('DELETE')
                            ...
                            <tr class="border-b dark:border-gray-700">
                                <th scope="row"
                                    ...
                                    {{ $loop->iteration }}</th>
                                <td class="px-4 py-3">{{ $data->pesanan_id }}</td>
                                <td class="px-4 py-3">{{ $data->pesanan->pelanggan->nama }}</td>
                                <td class="px-4 py-3">{{ $data->pesanan->tanggal }}</td>
            ...
            <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4"
                ...
                <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                    ...
                    dari
                    <span class="font-semibold text-gray-900 dark:text-white">{{ count($DetailPesanan) }}</span>
                </span>

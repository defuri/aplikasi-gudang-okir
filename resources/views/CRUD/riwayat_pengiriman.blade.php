        ...
        <div
                ...
                <div class="w-full md:w-1/2">
                    <form class="flex items-center focus-within:ring-1 focus-within:ring-blue-700 rounded-lg"
                        ...
                        <div class="relative w-full">
                            ...
                            <input type="text" value="riwayatPengiriman" name="tabel" id="" required class="hidden">
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3">id pengiriman</th>
                            <th scope="col" class="px-4 py-3">tanggal pengiriman</th>
                            <th scope="col" class="px-4 py-3">tanggal sampai</th>
                            <th scope="col" class="px-4 py-3">created at</th>
                            <th scope="col" class="px-4 py-3">updated at</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($riwayatPengiriman as $currentRiwayatPengiriman)
                            <tr class="border-b dark:border-gray-700">
                                <th scope="row"
                                    class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ $loop->iteration }}</th>
                                <td class="px-4 py-3">{{ $currentRiwayatPengiriman->id }}</td>
                                <td class="px-4 py-3">{{ $currentRiwayatPengiriman->pengiriman_id }}</td>
                                <td class="px-4 py-3">{{ $currentRiwayatPengiriman->tanggal_pengiriman }}</td>
                                <td class="px-4 py-3">{{ $currentRiwayatPengiriman->tanggal_sampai }}</td>
                                <td class="px-4 py-3">{{ $currentRiwayatPengiriman->created_at }}</td>
                                <td class="px-4 py-3">{{ $currentRiwayatPengiriman->updated_at }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            ...
            <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4"
                ...
                <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                    ...
                    dari
                    <span class="font-semibold text-gray-900 dark:text-white">{{ count($riwayatPengiriman) }}</span>
                </span>
                <ul class="inline-flex items-stretch -space-x-px">
                    <li>
                        {{ $riwayatPengiriman->links() }}
                    </li>
                </ul>
            </nav>
        </div>

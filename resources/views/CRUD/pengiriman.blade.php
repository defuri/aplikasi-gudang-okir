        ...
        <div
            ...
            <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                <div class="w-full md:w-1/2">
                    <form class="flex items-center focus-within:ring-1 focus-within:ring-blue-700 rounded-lg"
                        ...
                        <div class="relative w-full">
                            ...
                            <input type="text" value="pengiriman" name="tabel" id="" required class="hidden">
                ...
                <div
                    ...
                    <button type="button" data-modal-target="modalInsert" data-modal-toggle="modalInsert"
            ...
            {{-- * modal tambah data --}}
            <div id="modalInsert" tabindex="-1" aria-hidden="true"
                            ...
                            <div class="grid gap-4 mb-4 sm:grid-cols-2">
                                <div>
                                    <label for="pesanan_id"
                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Pilih ID
                                        pesanan:</label>
                                    <select id="pesanan_id" name="pesanan_id" required
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                        <option value="" disabled selected>Pilih ID Pesanan
                                        </option>
                                        @foreach ($pesanans as $currentPesanan)
                                            <option value="{{ $currentPesanan->id }}">{{ $currentPesanan->id }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        @foreach ($pengirimans as $currentPengiriman)
                            {{-- * ================================================================================================================================ --}}
                            {{-- ! modal update --}}

                            <div id="updateModal{{ $loop->iteration }}" tabindex="-1" aria-hidden="true"
                                class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
                                <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
                                    <!-- Modal content -->
                                    <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                                        ...
                                        <!-- Modal body -->
                                        <form action="{{ route('pengiriman.update', $currentPengiriman->id) }}"
                                            method="POST">
                                            @csrf
                                            @method('PUT')
                                            <div class="grid gap-4 mb-4 sm:grid-cols-2">
                                                <div>
                                                    <label for="datepickerUpdate{{ $loop->iteration }}"
                                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tanggal</label>
                                                    ...
                                                    <div class="relative max-w-sm">
                                                        ...
                                                        <input datepicker datepicker-autohide type="text" required
                                                            name="tanggal" id="datepickerUpdate{{ $loop->iteration }}"
                                                            value="{{ date('d-m-Y', strtotime($currentPengiriman->tanggal)) }}"
                                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                                            placeholder="Pilih tanggal">
                                                    </div>
                                                </div>
                                ...
                                <div class="relative p-4 w-full max-w-md max-h-full">
                                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                        ...
                                        <div class="p-4 md:p-5 text-center">
                                            ...
                                            <form action="{{ route('pengiriman.destroy', $currentPengiriman->id) }}"
                                                method="post">
                                                @csrf
                                                @method('DELETE')
                            ...
                            <tr class="border-b dark:border-gray-700">
                                <th scope="row"
                                    ...
                                    {{ $loop->iteration }}</th>
                                <td class="px-4 py-3">{{ $currentPengiriman->id }}</td>
                                <td class="px-4 py-3">{{ $currentPengiriman->pesanan_id }}</td>
                                <td class="px-4 py-3">{{ $currentPengiriman->tanggal }}</td>
                                <td class="px-4 py-3">{{ $currentPengiriman->created_at }}</td>
                                <td class="px-4 py-3">{{ $currentPengiriman->updated_at }}</td>
                                <td class="px-4 py-3 flex items-center justify-end">
                                    ...
                                    <div id="dropDownAksi{{ $loop->iteration }}"
                                        ...
                                        <ul class="py-1 text-sm text-gray-700 dark:text-gray-200"
                                            ...
                                            <li>
                                                <form
                                                    action="/owner/riwayat-pengiriman/{{ $currentPengiriman->id }}/selesai"
                                                    method="post">
                                                    @csrf
                                                    <button type="submit"
                                                        class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white w-full text-left">Selesai</button>
                                                </form>
                                            </li>
            ...
            <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4"
                ...
                <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                    ...
                    dari
                    <span class="font-semibold text-gray-900 dark:text-white">{{ count($pengirimans) }}</span>
                </span>

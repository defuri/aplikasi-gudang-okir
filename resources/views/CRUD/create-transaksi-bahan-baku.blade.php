...
<section class="-mt-14 bg-white dark:bg-gray-900">
    <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
        ...
        <form action="{{ route('create-transaksi-bahan-baku.store') }}" method="POST">
            @csrf
            {{-- date picker --}}
                ...
                <div class="w-full">
                    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                        ...
                        <select name="bahan_baku_id[]" required
                            class="bahan_baku bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 mt-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                            <option value="" selected disabled>Pilih bahan baku</option>

                            @foreach ($bahanBaku as $currentBahanBaku)
                                <option value="{{ $currentBahanBaku->id }}">{{ $currentBahanBaku->nama }}</option>
                            @endforeach
                        </select>
                    </label>
                </div>
                ...
                <div class="w-full">
                    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                        ...
                        <select name="satuan_id[]" required
                            class="satuan bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 mt-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                            <option value="" selected disabled>Pilih bahan baku</option>

                            @foreach ($satuan as $currentSatuan)
                                <option value="{{ $currentSatuan->id }}">{{ $currentSatuan->nama }}</option>
                            @endforeach
                        </select>
                    </label>
                </div>
            ...
            <p class="text-sm text-gray-900 dark:text-white text-right cursor-pointer" id="teksTambahTransaksi">Tambah
                Transaksi</p>
